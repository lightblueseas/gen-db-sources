...
public class GeneratorExtensions {
	...
	public static void generateRepositoryClasses() throws Exception {
		...
		final List<RepositoryClassModel> repositoryModels = getRepositoryClassModels(generationData);
	 ...
	 */
	public static ClassGenerationModel loadClassGenerationModel(
	...
	/**
	 * Initialize qualified model class names.
	 *
	...
	public static void initializeQualifiedModelClassNames(
			final ClassGenerationModel generationData) throws Exception {
		final Set<String> qualifiedModelClassNames =
			PackageExtensions.scanClassNames(generationData.getModelPackageName());
	...
	public static void generateClasses(final ClassGenerationModel generator,
		...
		for (final RepositoryClassModel model : repositoryModels) {
			...
			final Template repositoryClassTemplate = Velocity.getTemplate(generator
					.getRepositoryClassTemplateFile());

			final String repositoryClassPath = generator.getSrcFolder()
					+ generator.getSrcGenerationPackage()
			...
			final Template daoTestClassTemplate = Velocity.getTemplate(generator
					.getRepositoryTestClassTemplateFile());
			final String daoTestClassPath = generator.getSrcTestFolder()
					+ generator.getSrcTestGenerationPackage()
					+ model.getRepositoryClassName()  + FileSuffix.TEST
			...
			final Template serviceInterfaceTemplate = Velocity.getTemplate(generator
					.getServiceInterfaceTemplateFile());
			...
			final BufferedWriter serviceInterfaceWriter = new BufferedWriter(
					new FileWriter(generator.getSrcFolder()
							+ generator.getSrcServiceGenerationPackage()
							+ model.getServiceClassName() + FileExtension.JAVA.getExtension()));
			...
			final Template serviceClassTemplate = Velocity.getTemplate(generator
					.getServiceClassTemplateFile());
			final String serviceClassPath =generator.getSrcFolder()
					+ generator.getSrcServiceGenerationPackage()
					+ model.getModelClassName() + "Business"
	...
	public static List<RepositoryClassModel> getRepositoryClassModels(
		...
		for (final String clazz : generator.getQualifiedModelClassNames()) {
			...
			try {
				...
				final Class primaryKeyClassType = TypeArgumentsExtensions.getFirstTypeArgument(ClassExtensions.getBaseClass(modelClass), modelClass);
			...
			model.setModelInstanceName(StringExtensions
				.firstCharacterToLowerCase(model.getModelClassName()));
		}
		return repositoryModels;
	}

	public static void generatePoms() {
